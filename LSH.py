...
def timeTest(cols,n,normalize = True):
    '''
    q = 4, (b, r) = (100, 2) , s >= 0.1, DS = old (1199)
    ...
    q = 4, (b, r) = (20, 2) , s >= 0.22   DS = old (975)
    n       time        sig.size    bucket.size     pairs.size
    100     73s         31          564             10
    200     294s        68          1172            87
    
    q = 6, (b, r) = (20, 7) , s >= 0.65   DS = old (975)
    n       time        sig.size    bucket.size     pairs.size
    100     176s        23          420             3
    200     450s        53          997             7
    ...
    q = 5, (b, r) = (20, 7) , s >= 0.65,  DS = new (379)
    n       time        sig.size    bucket.size     pairs.size
    379     699s        218         4204            14
    
    q = 4, (b, r) = (20, 2) , s >= 0.22,  DS = new (379)
    n       time        sig.size    bucket.size     pairs.size
...
def filter_pairs(pairs):
    ...
    filename = {}
    for key in pairs:
        left = '%ELL ' in key[0][1] or '#ELL ' in key[0][1] \
            or '#Grade' in key[0][1]
        right = '%ELL ' in key[1][1] or '#ELL ' in key[1][1] \
            or '#Grade' in key[1][1]
        if  left and right:
            #filename[key[0][0]] = True
            #filename[key[1][0]] = True
            filename[key[1]] = True
            filename[key[0]] = True
            pass 
        else:
...
normCols_6 = getNormCols(cols,6)
pairs = timeTest(normCols_6,100,False)

pairs = timeTest(cols,503)
...
#FindSimilarColumns_LSH(sample_cols,20,7,0.65,5,normalize)
